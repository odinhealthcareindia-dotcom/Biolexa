...
function Section({ imageSrc, imageAlt, title, reverse = false, children }: SectionProps) {
  return (
    ...
    >
      <div className={`flex flex-col ${reverse ? "md:flex-row-reverse" : "md:flex-row"} items-center gap-8 max-w-7xl mx-auto`}>
        <motion.div
        ...
        <motion.div
          ...
          viewport={{ once: true, amount: 0.3 }}
          transition={{ duration: 0.6, ease }}
          className={`md:w-1/2 w-full ${reverse ? "md:pr-8" : "md:pl-8"}`}
        >
